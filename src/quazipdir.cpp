...
namespace QuaZipFileInfo64Sort
{
...
{
    if (sortFlags & QDir::LocaleAware)
    {
        if (sortFlags & QDir::IgnoreCase)
        {
    ...
    {
        return string1.compare(string2, (sortFlags & QDir::IgnoreCase)
                               ? Qt::CaseInsensitive : Qt::CaseSensitive);
    }
}

bool lessThan(const QuaZipFileInfo64 &info1, const QuaZipFileInfo64 &info2)
{
    QDir::SortFlags order = sortFlags & (QDir::Name | QDir::Time | QDir::Size | QDir::Type);
    if ((sortFlags & QDir::DirsFirst) == QDir::DirsFirst
            || (sortFlags & QDir::DirsLast) == QDir::DirsLast)
    {
        if (info1.name.endsWith('/') && !info2.name.endsWith('/'))
        {
            return (sortFlags & QDir::DirsFirst) == QDir::DirsFirst;
        }
        else if (!info1.name.endsWith('/') && info2.name.endsWith('/'))
        {
            return (sortFlags & QDir::DirsLast) == QDir::DirsLast;
        }
    }
    bool result;
    int extDiff;
    ...
    default:
        qWarning("QuaZipDirComparator(): Invalid sort mode 0x%2X",
                 static_cast<unsigned>(sortFlags));
        return false;
    }
    return (sortFlags & QDir::Reversed) ? !result : result;
}
} // end namespace QuaZipFileInfo64Sort
                                     ...
                                     QDir::Filters filter, QDir::SortFlags sort, TFileInfoList &result) const
{
    ...
    if (srt != QDir::NoSort && (srt & QDir::Unsorted) != QDir::Unsorted) {
        if (QuaZip::convertCaseSensitivity(caseSensitivity)
                == Qt::CaseInsensitive)
            srt |= QDir::IgnoreCase;
        QuaZipFileInfo64Sort::sortFlags = srt;
        std::sort(list.begin(), list.end(), QuaZipFileInfo64Sort::lessThan);
    }
